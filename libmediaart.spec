#
# Conditional build:
%bcond_with	qt		# use Qt5 instead of GdkPixbuf for media extraction
...
%build
%meson \
	-Dimage_library=%{?with_qt:qt5}%{!?with_qt:gdk-pixbuf} \
	-Dintrospection=true \
	%{?with_vala:-Dvapi=true}%{!?with_vala:-Dvapi=false} \
	-Dtests=false \
	-Dgtk_doc=false
...
%ghost %{_libdir}/libmediaart-2.0.so.0
